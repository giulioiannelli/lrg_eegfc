"""LRG analysis visualizations - REVISED based on FIGMNTGN notebooks.

This module provides correct implementations following:
- FIGMNTGN01.ipynb: PSI calculation and 4-panel layout
- FIGMNTGN03.ipynb: Dendrogram settings (tmin, tmax, log scale)
"""
...
def plot_lrg_full_panel(
    ...
    # Build graph from FC matrix
    G = nx.from_numpy_array(fc_matrix)

    # Compute network layout (k=0.1 for weighted networks from FIGMNTGN03)
    ...
    if len(edge_weights) > 0:
        ...
        if wmax > wmin:
            ...
            widths_scaled = [0.2] * len(edge_weights)
    else:
        widths_scaled = []
